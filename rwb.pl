...
#print $outputcookiecontent;
#print $outputdebugcookiecontent;
#
#
...
print "<div id=\"data_filter\"><input type=\"checkbox\" name=\"fec_data\" value=\"committees\" onClick=\"checkCheckbox()\" checked>Committees<br>";
 
print "<input type=\"checkbox\" name=\"fec_data\" value=\"candidates\" onClick=\"checkCheckbox()\" checked>Candidates<br>";
print "<input type=\"checkbox\" name=\"fec_data\" value=\"individuals\" onClick=\"checkCheckbox()\" checked>Individuals<br>";

#if (UserCan($user,"query-opinion-data")) {

print "<input type=\"checkbox\" name=\"fec_data\" value=\"opinions\" onClick=\"checkCheckbox()\" checked>Opinions";
...
my $formid = "allCycles";
my $cycleCheckboxes = MakeCheckboxes($formid, ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.committee_master UNION select distinct cycle from cs339.candidate_master UNION select distinct cycle from cs339.individual", undef));
print $cycleCheckboxes;

print "<button onClick=\"ViewShift()\">Apply</button>";
...
#$cycle = "('8384')";
  ...
  if ($what{committees}) { 
    ...
    my @comm_cand_count;
    eval {
      @comm_cand_count = ExecSQL($dbuser, $dbpasswd, "select count(*) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where CYCLE IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
    };
    my @comm_comm_count;
    eval {
      @comm_comm_count = ExecSQL($dbuser, $dbpasswd, "select count(*) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where CYCLE IN " .$cycle . " and latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
    };
    my $comm_count = @{$comm_cand_count[0]}[0] + @{$comm_comm_count[0]}[0];

    my $i = 0;
    while (($comm_count < 5) && ($i<5)) {
      $latne += 0.02;
      $longne += 0.02;
      $latsw -= 0.02;
      $longsw -= 0.02;
      ...
      $comm_count = @{$comm_cand_count[0]}[0] + @{$comm_comm_count[0]}[0];
      $i += 1;
    }
    if ($comm_count >= 5) {

        print "<div id='comm_trans_amnt'>"; 
	print "hi";
        my ($str2, $error2) = GetDemTrans($latne,$longne,$latsw,$longsw,$cycle,$format); 
        if (!$error2) {
          if ($format eq "table") {
            print "<h2>Democratic Committee to Candidate Transaction Amount</h2>$str2";
          } else {

            print "<h2>Democratic Committee to Candidate Transaction Amount</h2>";
            print $str2;
          }
        }
        my ($str3, $error3) = GetRepTrans($latne,$longne,$latsw,$longsw,$cycle,$format);
        if (!$error3) {
          if ($format eq "table") {
            print "<h2>Republican Committee to Candidate Transaction Amount</h2>$str3";
          } else {

            print "<h2>Republican Committee to Candidate Transaction Amount</h2>";
            print $str3;
          }
        }
        ##########################
        my ($str4, $error4) = GetDemCommTrans($latne,$longne,$latsw,$longsw,$cycle,$format);
        if (!$error4) {
          if ($format eq "table") {
            print "<h2>Democratic Committee to Committee Transaction Amount</h2>$str3";
          } else {

            print "<h2>Democratic Committee to Committee Transaction Amount</h2>";
            print $str4;
          }
        }
        my ($str5, $error5) = GetRepCommTrans($latne,$longne,$latsw,$longsw,$cycle,$format);
        if (!$error5) {
          if ($format eq "table") {
            print "<h2>Republican Committee to Committee Transaction Amount</h2>$str3";
          } else {

            print "<h2>Republican Committee to Committee Transaction Amount</h2>";
            print $str5;
          }
        }
        ##############################

        my $rep_dem_comm_diff = $str2 - $str3 + $str4 - $str5;
        my $rep_dem_comm_trans = abs $rep_dem_comm_diff;
        print "<div id='diff'>$rep_dem_comm_trans</div>";
        print "</div>";
    }
}
  ...
  if ($what{individuals}) {
    ...
    my @ind_sql;
    eval {
      @ind_sql = ExecSQL($dbuser, $dbpasswd, "select count(*) from cs339.individuals natural join cs339.ind_id_to_geo where CYCLE IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
    };
    my $ind_count = @{$ind_sql[0]}[0];

    my $i = 0;
    while (($ind_count < 5) && ($i<5)) {
      $latne += 0.02;
      $longne += 0.02;
      $latsw -= 0.02;
      $longsw -= 0.02;
      ...
      $ind_count = @{$ind_sql[0]}[0];
      $i += 1;
    }
    if ($ind_count >= 5) {
      print "<div id='indi_trans_amnt'>";
      my ($str2,$error2) = GetIndiTrans($latne,$longne,$latsw,$longsw,$cycle,$format);
      if (!$error2) {
        if ($format eq "table") {
          print "<h2>Individual Transaction Amount</h2>$str2";
        } else {
  ...
  if ($what{opinions}) {
    ...
    my @opinions_sql;
    eval {
      @opinions_sql = ExecSQL($dbuser, $dbpasswd, "select COUNT(*) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
    };
    my $opinions_count = @{$opinions_sql[0]}[0];

    my $i=0;
    while (($opinions_count < 5) && ($i < 5)) {
      $latne += 0.02;
      $longne += 0.02;
      $latsw -= 0.02;
      $longsw -= 0.02;
      ...
      $opinions_count = @{$opinions_sql[0]}[0];
      $i += 1;
    }

    print "<div id='opinions_data'>";
    my ($str2,$error2) = GetOpinionsData($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error2) {
      if ($format eq "table") {
        print "<h2>Opinions Data</h2>$str2";
      } else {

        print "<h2>Opinions Data</h2>";
	print "Avg \tStdev";        
	print $str2;
      }
    }
...
#	my @cycles = split /,/,$cycle;
#	my $cyclesQ = '\'' . join('\',\'',@cycles) . '\'';
	#my $cyclesQ = sprintf '\'%s\'' x @cycles, @cycles;
#	print $cyclesQ;
 ...
 #     print $cycle; 
    ...
    if (!$error2) {
      if ($format eq "table") {
        print "<h2>Democratic Transaction Amount</h2>$str2";
      } else {
        print $str2;
      }
    }
    ...
    if (!$error3) {
      if ($format eq "table") {
        print "<h2>Republican Transaction Amount</h2>$str3";
      } else {
        print $str3;
      }
    }
    my $rep_dem_comm_diff = $str2 - $str3;
    my $rep_dem_comm_trans = abs $rep_dem_comm_diff;
    print "<div id='diff'>$rep_dem_comm_trans</div>";
    print "</div>";
...
if ($action eq "register") { 
   if(!$run){
	...
	h2('Register'),
	  "Name: ", textfield(-name=>'name'),
		...
		p,
		  "Password: ", textfield(-name=>'password'),
		    p,
			"Access Code: ", textfield(-name=>'code'),
			p,
      ...
      $error = Register($name, $email, $password, $code);	
#	if(!$error){
...
if ($action eq "give-opinion-data") { 
   ...
   if(!$run){
	...
	FindMe = function(){
		...
		console.log(\"JOIADJIOAOJFFJIODF FFEFEFEF HAHAH  \");
		
		lat = encodeURIComponent(lat);
		long = encodeURIComponent(long);
		var string_url = 'https://murphy.wot.eecs.northwestern.edu/~drj132/rwb/rwb.pl?act=give-opinion-data' + '&latitude=' + lat + '&longitude=' + long;
		if (window.location.href != string_url){
	...
	printStuff = function(){console.log(\"hello\");}
...
my $long = param("longitude");
     my %opmap = (
      ...
      #print "<input type=\"button\" onClick=\"getCurrLocation()\"></input>";

	print "<pre id=\"wait_msg\">Please wait a few seconds to allow us to retrieve your current location</pre>";

      print start_form(-name=>'GiveOpinion'),
	h2('Give your Opinion'),
	  "Assign political color : ", popup_menu(
			-name=>'opinion',
			-values=>[sort keys %opmap],
			-labels=>\%opmap,
			),
		hidden(-name=>'lat',-values=>$lat),
		hidden(-name=>'long',-values=>$long),
		      hidden(-name=>'run',-default=>['1']),
	...
	AddOpinion($user, $color, $latitude, $longitude);	
...
##############
#print ExecSQL($dbuser, $dbpasswd, "select cmte_nm, cmte_zip, cycle from cs339.committee_master where cycle IN (?)",undef, '\'8384\'');

#print MakeRaw(ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.committee_master UNION select distinct cycle from cs339.candidate_master UNION select distinct cycle from cs339.individual", undef));
...
sub Committees {
 ...
 #print MakeRaw("test", "2D", ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_id_to_geo where cycle IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<?",undef, $latsw,$latne,$longsw,$longne));
 #print MakeRaw("test", "2D", ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle IN ? and latitude>? and latitude<? and longitude>? and longitude<?",undef, $cycle, $latsw,$latne,$longsw,$longne));
 print "</h3>"; 
#print MakeRaw( ExecSQL($dbuser, $dbpasswd, "select cmte_nm, cmte_zip, cycle from cs339.committee_master where cycle IN (?)",undef, $cycle));
  my @rows;
  eval { 
...
sub Opinions {
  ...
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, color, submitter from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
  };
...
sub Register{
	my @inviter;
	my ($name, $email, $password, $code) = @_;
	
	# Check if access code is in rwb_invite table and get inviter
	eval {@inviter =  ExecSQL($dbuser, $dbpasswd, "select inviter from rwb_invite where code = ?", "COL", $code);};
#	print $@;
	print @inviter;

	# Query db for permissions of inviter
	if (@inviter){
		#First add user to db
						
		my $invite = join('', @inviter);
		print $invite;
  		eval { ExecSQL($dbuser,$dbpasswd,
		 "insert into rwb_users (name,password,email,referer) values (?,?,?,?)",undef,$name, $password, $email, $invite);};
		
	
		my @perms;
		eval { @perms = ExecSQL($dbuser, $dbpasswd, "select action from rwb_permissions where name=?", "COL", $invite);};

		foreach my $perm(@perms){
			GiveUserPerm($name, $perm);
		} 
				
		eval { ExecSQL($dbuser, $dbpasswd, "delete from rwb_invite where code = ?", undef, $code);};
	}
...
# WRITE ACCESS CODE TO FILE
	...
	open(my $filehandler, '>', $filename) or die "Could not open file";
	print $filehandler "Your access code is $str \n\n Please register at \n https://murphy.wot.eecs.northwestern.edu/~drj132/rwb/rwb.pl?act=register";
	close $filehandler;
	print "done";	
	
# SEND EMAIL
	$mail_status = `cat mail.txt | mail -s "This is a test" $email`;
	if (!$mail_status)
	{
		eval { ExecSQL($dbuser, $dbpasswd, "insert into rwb_invite (code, inviter) values (?, ?)", undef, $str, $inviter); };
...
sub GetDemTrans {
  ...
  my @rows;
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select SUM(TRANSACTION_AMNT) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where cycle IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<? AND cmte_id IN (select cmte_id from cs339.committee_master where CMTE_PTY_AFFILIATION='DEM')",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("dem_trans_amnt", "2D",
      ["democrat transaction amount"],
      @rows),$@);
    } else {
        return (MakeRaw("dem_trans_amnt","2D",@rows),$@);
    }
  }
}
...
sub GetRepTrans {
  ...
  my @rows;
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select SUM(TRANSACTION_AMNT) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where cycle IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<? AND cmte_id IN (select cmte_id from cs339.committee_master where CMTE_PTY_AFFILIATION='REP')",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("rep_trans_amnt", "2D",
      ["republican transaction amount"],
      @rows),$@);
    } else {
        return (MakeRaw("rep_trans_amnt","2D",@rows),$@);
    }
  }
}
...
sub GetDemCommTrans {
  ...
  my @rows;
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select SUM(TRANSACTION_AMNT) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where cycle IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<? AND cmte_id IN (select cmte_id from cs339.committee_master where CMTE_PTY_AFFILIATION='DEM')",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("dem_comm_trans_amnt", "2D",
      ["democrat transaction amount"],
      @rows),$@);
    } else {
        return (MakeRaw("dem_comm_trans_amnt","2D",@rows),$@);
    }
  }
}
...
sub GetRepCommTrans {
  ...
  my @rows;
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select SUM(TRANSACTION_AMNT) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where cycle IN " . $cycle . " and latitude>? and latitude<? and longitude>? and longitude<? AND cmte_id IN (select cmte_id from cs339.committee_master where CMTE_PTY_AFFILIATION='REP')",undef,$latsw,$latne,$longsw,$longne);
  };
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("rep_comm_trans_amnt", "2D",
      ["democrat transaction amount"],
      @rows),$@);
    } else {
        return (MakeRaw("rep_comm_trans_amnt","2D",@rows),$@);
    }
  }
}
...
sub GetIndiTrans {
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("indi_trans_amnt", "2D",
      ["individual transaction amount"],
      @rows),$@);
    } else {
        return (MakeRaw("indi_trans_amnt","2D",@rows),$@);
    }
  }
}
...
sub GetOpinionsData {
  ...
  if ($@) {
    ...
    if ($format eq "table") {
      return (MakeTable("opinions_data", "2D",
      ["opinions data"],
      @rows),$@);
    } else {
        return (MakeRaw("opinions_data","2D",@rows),$@);
    }
  }
}
...
sub MakeCheckboxes{
	...
	foreach my $cycle (@list){
		$out.="<input type=\"checkbox\" name=\"cycles\" value=\"@{$cycle}\" onClick=\"checkCycles()\" checked>@{$cycle}"; 
	}
	$out.="</form>\n";
	#print $out;
	return $out;
